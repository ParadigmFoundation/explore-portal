...
class TableTransaction extends PureComponent { 
  ...
  onClickTableHeader = (col) => {
    ...
    if(col == curSortIndex) 
      this.setState({
        tableData: arraySort(tableData, colNames[curSortIndex], {reverse:curSortState}),
        curSortState: !curSortState
      });
    else 
      this.setState({
        tableData:    arraySort(this.props.tableData, colNames[col], {reverse:!curSortState}),
        curSortIndex: col, 
        curSortState: true
      });
  }
              ...
              <tr>
                <th width='1000px'>
                  <div style={{minWidth:'200px'}}>
                    <span onClick={()=>this.onClickTableHeader(0)}>ID</span>
                    {this.sortImage(0)}
                  </div>
                </th>
                <th width='1000px'>
                  <div style={{minWidth:'200px'}}>
                    <span onClick={()=>this.onClickTableHeader(1)}>Poster</span>
                    {this.sortImage(1)}
                  </div>
                </th>
                <th width='1000px'>
                  <div style={{minWidth:'200px'}}>
                    <span onClick={()=>this.onClickTableHeader(2)}>Type</span>
                    {this.sortImage(2)}
                  </div>
                </th>
              </tr>
