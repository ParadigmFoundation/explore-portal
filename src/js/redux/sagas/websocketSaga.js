...
import {
  constants as webSocketConstants,
    ...
    while (true) {
      ...
      if (message) {
        ...
        setInterval(() => {
          ...
          if (
            mySocket.readyState === WebSocket.CLOSING ||
...
export default function* watchWebSocket() {
  yield [
    takeLatest(
      webSocketConstants.WEBSOCKET_CONNECT,
      initializeWebSocketsChannel
    )
  ];
}
export const websocketSaga = [fork(watchWebSocket)];
